...
namespace ImGui
{
    ...
    // - The disk functions are automatically called if io.IniFilename != NULL (default is "imgui.json").
    // - Set io.IniFilename to NULL to load/save manually. Read io.WantSaveIniSettings description about handling .json saving manually.
    IMGUI_API void          LoadIniSettingsFromDisk(const char* ini_filename);                  // call after CreateContext() and before the first call to NewFrame(). NewFrame() automatically calls LoadIniSettingsFromDisk(io.IniFilename).
    IMGUI_API void          LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size=0); // call after CreateContext() and before the first call to NewFrame() to provide .json data from your own data source.
    IMGUI_API void          SaveIniSettingsToDisk(const char* ini_filename);                    // this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .json file (and also by DestroyContext).
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated string with the .json data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoSavedSettings        = 1 << 8,   // Never load/save settings in .json file
...
enum ImGuiCond_
{
    ...
    ImGuiCond_FirstUseEver  = 1 << 2,   // Set the variable if the object/window has no persistently saved data (no entry in .json file)
...
struct ImGuiIO
{
    ...
    float       IniSavingRate;                  // = 5.0f           // Minimum time between saving positions/sizes to .json file, in seconds.
    const char* IniFilename;                    // = "imgui.json"    // Path to .json file. Set NULL to disable automatic .json loading/saving, if e.g. you want to manually load/save from memory.
    ...
    bool        WantSaveIniSettings;            // When manual .json load/save is active (io.IniFilename == NULL), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. IMPORTANT: You need to clear io.WantSaveIniSettings yourself.
